...
from tools.utils import get_logger, save_checkpoint, set_seed
...
def parse_option():
    ...
    # Miscs
    parser.add_argument(
        "--output", type=str, help="your output path to save model and logs"
    )
...
def main(rank, world_size, config):
    ...
    if config.DATA.DATASET == "prcc":
        ...
        trainloader, queryloader, galleryloader, dataset, train_sampler = (
            build_dataloader(config)
        )

    # Build model
    model = build_model(config, dataset.num_train_pids)
    ...
    logger.info(
        "Total Model size: {:.5f}M".format(
            sum(p.numel() for p in parameters) / 1000000.0
        )
    )
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        if (
            (epoch + 1) > config.TEST.START_EVAL
            and config.TEST.EVAL_STEP > 0
            and (epoch + 1) % config.TEST.EVAL_STEP == 0

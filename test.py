...
def test(config, model, queryloader, galleryloader, dataset):
    ...
    model.eval()
    # Extract features
    qf, q_pids, q_camids, q_clothes_ids = extract_img_feature(
        config, model, queryloader
    )
    gf, g_pids, g_camids, g_clothes_ids = extract_img_feature(
        config, model, galleryloader
    )
    # Gather samples from different GPUs
    torch.cuda.empty_cache()
    qf, q_pids, q_camids, q_clothes_ids = concat_all_gather(
        [qf, q_pids, q_camids, q_clothes_ids], len(dataset.query)
    )
    ...
    logger.info(
        "Extracted features for gallery set, obtained {} matrix".format(gf.shape)
    )
    logger.info(
        "Extracting features complete in {:.0f}m {:.0f}s".format(
    ...
    logger.info(
        "Distance computing in {:.0f}m {:.0f}s".format(
            time_elapsed // 60, time_elapsed % 60
        )
    )
    since = time.time()
...
def test_prcc(
    ...
    qsf, qs_pids, qs_camids, qs_clothes_ids = extract_img_feature(
        config, model, queryloader_same
    )
    qdf, qd_pids, qd_camids, qd_clothes_ids = extract_img_feature(
        config, model, queryloader_diff
    )
    ...
    time_elapsed = time.time() - since
    logger.info(
        "Extracted features for query set (with same clothes), obtained {} matrix".format(
            qsf.shape
        )
    )
    logger.info(
        "Extracted features for query set (with different clothes), obtained {} matrix".format(
            qdf.shape
        )
    )
    logger.info(
    ...
    qs_pids, qs_camids, qs_clothes_ids = (
        qs_pids.numpy(),
        qs_camids.numpy(),
        qs_clothes_ids.numpy(),
    )
    qd_pids, qd_camids, qd_clothes_ids = (
        qd_pids.numpy(),
        qd_camids.numpy(),
        qd_clothes_ids.numpy(),
    )
    g_pids, g_camids, g_clothes_ids = (
        g_pids.numpy(),
        g_camids.numpy(),
